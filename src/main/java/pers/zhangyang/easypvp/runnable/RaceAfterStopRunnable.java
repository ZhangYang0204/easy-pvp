...
public class RaceAfterStopRunnable extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (rest>0){
            ...
            String title= MessageYaml.MESSAGE_YAML_MANAGER.getTITLE_CELEBRATE_TIME_TITLE();
            if (title!=null){
                title= ReplaceUtil.replace(MessageYaml.MESSAGE_YAML_MANAGER.getTITLE_CELEBRATE_TIME_TITLE(),
                        Collections.singletonMap("{time}",String.valueOf(rest)));
            }
            String stitle=MessageYaml.MESSAGE_YAML_MANAGER.getTITLE_CELEBRATE_TIME_SUBTITLE();
            if (stitle!=null){
                stitle=ReplaceUtil.replace(MessageYaml.MESSAGE_YAML_MANAGER.getTITLE_CELEBRATE_TIME_SUBTITLE(),
                        Collections.singletonMap("{time}",String.valueOf(rest)));
            }
