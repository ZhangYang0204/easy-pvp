...
public class CommandAddMapDescription extends CommandBase {
    ...
    protected boolean run() {
        ...
        try {
            ...
            List<String> list= MessageYaml.INSTANCE.getCHAT_FAILURE_ADD_MAP_DESCRIPTION_BECAUSE_NOT_EXIST_MAP_NAME();
            if (list!=null) {
        ...
        rep.put("{map}",args[1]);
        rep.put("{description}",args[2]);
        List<String> list= MessageYaml.INSTANCE.getCHAT_SUCCESS_ADD_MAP_DESCRIPTION();
        if (list!=null) {
            ReplaceUtil.replace(list, rep);
        }
        MessageUtil.sendMessageTo(sender, list);
