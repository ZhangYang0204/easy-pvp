...
public class CompleterSetMapScale extends CompleterBase {
    public CompleterSetMapScale(CommandSender sender, boolean forcePlayer, String[] args) {
        super(sender, forcePlayer, args);
    }

    @Override
    public List<String> complete() {
        if (args.length==2){     List<String> list=MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_SCALE();
        ...
        if (args.length==3){
            return removeStartWith(args[2], MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_SCALE_$()
                    ==null?new ArrayList<>():MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_SCALE_$()
            );
        }
