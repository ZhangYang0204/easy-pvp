...
public class CompleterSetMapChooseTime extends CompleterBase {
    ...
    public List<String> complete() {
        if (args.length==2){
            List<String> list=MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_CHOOSE_TIME();
        ...
        if (args.length==3){
            return removeStartWith(args[2], MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_CHOOSE_TIME_$()
                    ==null?new ArrayList<>():MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_CHOOSE_TIME_$()
            );
        }
