...
public class CompleterMapBuildSet extends CompleterBase {
    ...
    public List<String> complete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length==2){
            return removeStartWith(args[1], MessageYamlManager.MESSAGE_YAML_MANAGER.getCOMPLETER_EASY_PVP_MAP_BUILD_SET());
        }
        if (args.length==3){
            return removeStartWith(args[2], MessageYamlManager.MESSAGE_YAML_MANAGER.getCOMPLETER_EASY_PVP_MAP_BUILD_SET_$());
        }
