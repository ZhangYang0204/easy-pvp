...
public class CompleterSetMapRaceTime extends CompleterBase {
    ...
    public @NotNull List<String> complete() {
        ...
        if (args.length==2){
            List<String> list= MessageYaml.INSTANCE.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_RACE_TIME();
        ...
        if (args.length==3){
            return removeStartWith(args[2], MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_RACE_TIME_$()
                    ==null?new ArrayList<>():MessageYaml.INSTANCE.getCOMPLETER_EASY_PVP_SET_MAP_RACE_TIME_$()
            );
        }
